    ...
    <form className="convertir-audio-form" action="http://localhost:8080/convert" method="post" encType="multipart/form-data" >
      <div className="dropzone-box">
        ...
        <div className="audio-formats">
          ...
          <ul id="format">
            {['mp3', 'wav', 'flac', 'aac', 'ogg', 'wma', 'alac', 'aiff', 'opus'].map((audioFormat) => (
              <li key={audioFormat}>
                <input
                  type="radio"
                  value={audioFormat}
                ...
                />
                <label htmlFor={audioFormat}>{audioFormat.toUpperCase()}</label>
              </li>
            ))}
          </ul>
        </div>
